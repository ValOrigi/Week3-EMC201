...
public class CharacterBehaviors : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown("z") && canShoot)
        {
            Instantiate(fireballPrefab, projOffset.position, projOffset.rotation);
        }
    ...
    {
        MovementState state;
        if (walkSpeed > 7.5f && inputHorizontal != 0f)
        {
            state = MovementState.running;
        }
        else if (walkSpeed == 7.5f && inputHorizontal != 0f)
        {
